...
旅行業界向けの商材造成システムでフルサイクルエンジニアとして、設計・開発から運用までを担当していました。主には、運用や問い合わせでの開発が多く運用負荷を下げることや他の人が実施できるようにドキュメントや共有する仕組みなどを試行錯誤していました。
2024年からは、少人数のチームリーダーをしていました。
...
- コードレビュー会
  - 入社当初コードにドメイン知識が含まれており、１人ではレビューできないためチームを巻き込んでレビューする会を発足した。
- LT会
  - 会社の人をよく知りたいという気持ちから、ラフに発表できるLT会を発足・運用をしました。
- 単体テスト布教会
  - もくもく会してた
- 気になる記事を自分の分報に流した
  - 要約をChatGPTに生成させた
  - Veinを導入したが、定着させることはできなかった
